...
def dividir_dataset(
    dados_df: DataFrame, feature_name: str, valor: float
...
def get_test_and_train_dataframes(
    path: str, label: str, features_to_ignore: list[str], test_proportion: float
) -> tuple[DataFrame, DataFrame]:
    dataset_completo = carregar_dados(
        path, features_to_ignore=features_to_ignore, label_col_name=label
    )
...
def _gerar_dot_recursivo(arvore, dot, node_id_counter):
    ...
    dot.node(
        current_id, label=label, shape="ellipse", style="filled", color="lightgray"
    )
...
def visualizar_rede_neural(nn, feature_names, class_labels, nome_arquivo="rede_neural"):
    ...
    with dot.subgraph(name="cluster_input") as c:
        c.attr(
            label="Camada de Entrada", rank="same", style="filled", color="lightgray"
        )
        c.attr(rank="min")
...
def class_labels(predicoes_nn, teste_df, nome_label, labels_ordenados):
    ...
    cm_df = DataFrame(
        cm,
        index=[f"Real: {name}" for name in nomes_classes_str],

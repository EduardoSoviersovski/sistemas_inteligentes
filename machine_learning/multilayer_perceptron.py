...
class RedeNeural:
    def __init__(self, n_entradas, n_ocultas, n_saidas):
        ...
        self.pesos_oculta = (
            np.random.randn(self.n_entradas, self.n_ocultas) * std_dev_oculta
        )
        self.bias_oculta = np.zeros(self.n_ocultas)
        self.pesos_saida = (
            np.random.randn(self.n_ocultas, self.n_saidas) * std_dev_saida
        )
    ...
    def feedforward(self, entradas_np):
        self.ativacoes_oculta_raw = (
            np.dot(entradas_np, self.pesos_oculta) + self.bias_oculta
        )
        self.saidas_oculta_atual = relu(self.ativacoes_oculta_raw)

        saidas_logits = (
            np.dot(self.saidas_oculta_atual, self.pesos_saida) + self.bias_saida
        )
    ...
    def treinar(self, dados_treino, dados_validacao):
        ...
        print(
            f"Iniciando treino com {n_amostras} amostras, Batch Size={BATCH_SIZE}, "
        ...
        for epoca in range(EPOCHS):
            ...
            for i in range(0, n_amostras, BATCH_SIZE):
                ...
                for linha in batch_data:
                    ...
                    grad_po, grad_bo, grad_ps, grad_bs = self.calcular_gradientes(
                        entradas_np, esperado_np
                    )
                ...
                self.v_pesos_oculta = (
                    MOMENTUM_BETA * self.v_pesos_oculta
                ) + grad_medio_po
                self.v_bias_oculta = (
                    MOMENTUM_BETA * self.v_bias_oculta
                ) + grad_medio_bo
                self.v_pesos_saida = (
                    MOMENTUM_BETA * self.v_pesos_saida
                ) + grad_medio_ps
...
if __name__ == "__main__":
    ...
    dataset_completo = carregar_dados(
        CAMINHO_ARQUIVO, features_to_ignore=FEATURES_TO_IGNORE, label_col_name=LABEL
    )
    ...
    dataset_completo[features_cols] = (
        dataset_completo[features_cols] - min_vals
    ) / range_vals

    treino_completo_df, teste_df = dividir_treino_teste(
        dataset_completo, TEST_PROPORTION
    )
    treino_nn_df, validacao_df = dividir_treino_teste(
        treino_completo_df, VALIDATION_PROPORTION
    )
    ...
    nn.treinar(dados_treino=treino_list_balanceada, dados_validacao=validacao_list)
    ...
    try:
        visualizar_rede_neural(
            nn, features_cols, labels_ordenados, nome_arquivo="rede_neural_graph_v2"
        )

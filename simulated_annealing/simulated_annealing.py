...
from commons.utils import (export_results_to_csv, plot_results_scatterplot, route_length, two_opt_swap, Route, load_points_from_csv,
        ...
        for _ in range(ITERS_PER_T):
            ...
            if delta < 0 or (rng.random() < math.exp(-delta / T)):
                ...
                if current_cost < best_cost:
                    best, best_cost = current[:], current_cost
        T = schedule_handler(T0, T, ITERS_PER_T, iteration, ALPHA, LOG_ALPHA, BETA, schedule)
        iteration += 1
    return best, best_cost

def schedule_handler(t0: float, t: float, iters_per_t:int, iteration: int, alpha: float,log_alpha:float, beta:float, schedule:str) -> float:
...
def main():
    ...
    for schedule in cooling_schedule:
        for configuration in configurations:
            print(f"Executando: Schedule='{schedule}', Config='{configuration['name']}'...")
            for i in range(10): 
                start_time = time.time()
                route, length = simulated_annealing(dist_matrix, configuration, schedule)
                end_time = time.time()
                ...
                print(f"  -> Execução {i+1:02d}: Comprimento = {length:.4f}, Tempo = {execution_time:.4f}s")
    ...
    if PLOT:
        ...
        best_title = (f"Melhor Rota Geral (Comprimento: {best_overall_result['length']:.2f})\n"
        ...
        worst_title = (f"Pior Rota Geral (Comprimento: {worst_overall_result['length']:.2f})\n"
        ...
        for schedule in cooling_schedule:
            ...
            results_for_schedule = [r for r in all_run_results if r['schedule'] == schedule]
            
            # Cria o título para o gráfico
            scatter_title = f"Resultados das Configurações (Schedule: {schedule.capitalize()})"
